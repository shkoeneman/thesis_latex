...
{
The use of likelihood-based information criteria such as AIC, BIC, and AICc for model selection is commonplace. When using such
criteria to compare candidate models, lower values of a criterion typically indicate evidence of better model fit. Model 
...
Another principle in using likelihood-based information criteria for model selection can be dubbed the "Rule of 2." This rule
of thumb recommends that one should not discount models that possess a criterion value within 2 of the minimum criterion value among
all candidate models, as opposed to simply choosing the minimum criterion value. This rule can be shown to improve the performance
of model selection using information criteria in finite sample sizes if first used to identify models that are serious candidates for
selection followed by selection of one of these models using some other convention.

However, while effective, the "Rule of 2" possesses limited theoretical backing. Its justification largely lies on empirical simulations
...
by more formal probability theory. Through simulations, this procedure is shown to be able to produce better efficacy in description and
prediction in model selection as compared to other methods using the same information criterion. Additionally, a novel goodness-of-fit test
...
{
Model selection is a task often undertaken by statisticians and non-statisticians alike. The goals of model selection may be prediction of future
...
One such method of model selection involves the use of likelihood-based information criteria such as AIC, BIC, and AICc. These criteria help one
...
a criterion value within 2 of the minimum criterion value among all candidate models. When applying the principles of this rule, one can often
find good efficacy in model selection.
...
and distributional theory. Using distributional results, a new model selection algorithm to select a model from a collection of candidates is presented, along
with a novel goodness-of-fit test for linear models. The efficacy of these methods are demonstrated in simulation studies, and additionally an example
