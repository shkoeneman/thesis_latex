...
\Chapter{DISTRIBUTION-BASED MODEL SELECTION PROCEDURE}
		...
		This procedure is limited to considering a group of candidate models that are all of the same parametric class, with the class containing a largest candidate
		...
		dissimilar to the case of using just AIC on its own, as AIC also is only an asymptotically unbiased estimator of the expected Kullback–Leibler discrepancy. In the
		Appendix, a modification to this procedure is demonstrated that can be used in the case of nested normal linear regression models that employs the distributional
		properties of the F-Test to remove the reliance on the asymptotics of the likelihood ratio test (Allen, 1998). However, this version of the procedure is both limited
		to only normal linear regression and was found to provide only marginal benefit in small sample sizes for a substantial cost in computational complexity, and thus
		will not be expanded upon beyond the reference in the Appendix. However, it is presented for completeness.
		...
		\begin{equation}
			AICc_i - AICc_* = (-2 \ell (\hat{\theta}_i) + 2 \ell (\hat{\theta}_*) ) + (2(p-k_i)(\frac{n}{n-p+k_i-1}) - 2p(\frac{n}{n-p-1})) .
		\end{equation}
		...
		Note also the form of this difference, in that it is a difference in random goodness-of-fit terms along with a penalty term that is constant with respect to the
		...
		\end{equation}
		will hold for a sufficient sample size. Thus, under the same assumption, the approximate relation
		\begin{equation}
			\begin{split}
				\frac{AICc_i - AICc_*}{\sqrt{2k_i}} - \frac{(2(p-k_i)(\frac{n}{n-p+k_i-1}) - 2p(\frac{n}{n-p-1}))}{\sqrt{2k_i}} \, \dot\sim \, \\ 
		...
		\begin{equation}
			E \left[ \frac{AICc_i - AICc_*}{\sqrt{2k}} \right] \approx \sqrt{\frac{k_i}{2}} + \frac{(2(p-k_i)(\frac{n}{n-p+k_i-1}) - 2p(\frac{n}{n-p-1}))}{\sqrt{2k_i}}
		\end{equation}
		...
		\begin{definition}[Standardized AICc (Linear Regression) for model $M_*$]
			...
			$\max_{k_i} \sqrt{\frac{k_i}{2}} + \frac{(2(p-k_i)(\frac{n}{n-p+k_i-1}) - 2p(\frac{n}{n-p-1}))}{\sqrt{2k_i}}$.
		\end{definition}
		...
		\begin{algorithm}
			...
			\begin{algorithmic}[1]
			  %\Statex \textbullet~\textbf{Parameters:} $n, t \in \mathbb{N}$, where $t < n$.
			  \State Take steps to ensure with reasonable certainty that $M_*$ does not exhibit lack of fit.
			  \State For each distinct value of $k_i$, calculate $\sqrt{\frac{k_i}{2}} + \frac{(2(p-k_i)(\frac{n}{n-p+k_i-1}) - 2p(\frac{n}{n-p-1}))}{\sqrt{2k_i}}$
			  ...
			  $M_*$ as $\overline{AICc}_* = \max_{k_i} \left[ \sqrt{\frac{k_i}{2}} + \frac{(2(p-k_i)(\frac{n}{n-p+k_i-1}) - 2p(\frac{n}{n-p-1}))}{\sqrt{2k_i}} \right]$.
			  \State Categorize all models with a standardized AICc of
			  $ \max_{k_i} \left[ \sqrt{\frac{k_i}{2}} + \frac{(2(p-k_i)(\frac{n}{n-p+k_i-1}) - 2p(\frac{n}{n-p-1}))}{\sqrt{2k_i}} \right] + 2$
		...
		\begin{algorithm}
			...
			\begin{algorithmic}[1]
			  ...
			  $\overline{BIC}_i = \frac{BIC_i - BIC_*}{\sqrt{2k_i}}$. Define the standardized AIC for model
			  $M_*$ as $\overline{BIC}_* = \sqrt{\frac{1}{2}} (1-\log(n))$.
			  \State Categorize all models with a standardized AIC of $\sqrt{\frac{1}{2}} (1-\log(n)) + 2$ or lower as
			  being candidates for selection.
			  \State Among candidate models for selection, select the most parsimonious model, i.e. the model
		...
		This procedure will have many of the same advantages as the one involving BIC. However, it relies on an asymptotically consistent criterion over one that
		...
		The descriptive and predictive efficacy of each method, among each flavor of information criteria, will be evaluated after each method has selected a model.
		...
		\begin{algorithm}
			\caption{Model Selection Simulation to Compare Selected Normal Linear Regression Models}
			\begin{algorithmic}[1]
		...
		We now move onto the results that pertain to the AICc-based methods employed in this simulation. The following table and plot display the across across different values
		...
		In the case of these three criterion, we have seen that the new model selection procedures presented in this chapter can offer tangible advantages in terms of both description
		...
		\begin{equation}
			y_{\cdot i} \sim MVN( \mu_i \mathbf{1}_5, \Sigma) ,
		\end{equation}
		where $\mathbf{1}_5$ is a 5 by 1 vector of ones, the quantity
		\begin{equation}
		...
		with summary statistics to be calculated afterwards. A full summary of the simulation procedure can be found below.
		\begin{algorithm}
			\caption{Model Selection Simulation to Compare Selected Covariance Structures in a Longitudinal Model}
			\begin{algorithmic}[1]
		...
		Once again, we observe that all three methods do rather well as selecting the correct mean structure, but that the standardized AIC method performs the best by a slight margin. We will
		...
		We see a familiar pattern here as all methods again do relatively well in choosing the correct true model. However, the gaps in performance between each method are slightly more pronounced
		than in only the covariance or mean cases, likely on account of being akin to the sum of those individual gaps. Thus, while standardized AIC provides only a marginal boost in performance
