...
\Chapter{DISTRIBUTION-BASED MODEL SELECTION PROCEDURE}
		...
		rules of thumb that have been developed such as the "Rule of 2." While it was discussed that AIC, and by extension AICc, are asymptotically efficient criterion,
		...
		while it adds computational complexity; nevertheless, it remains an option if one prefers this method. One could use a rule other than parsimony to select a model
		...
		If one wishes to compare models across different parametric classes, a suggestion may be to determine through goodness-of-fit assessment which largest candidate model
