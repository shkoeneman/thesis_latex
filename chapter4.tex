...
\Chapter{GOODNESS-OF-FIT PROCEDURE FOR NORMAL LINEAR REGRESSION MODELS}
	...
	that can be employed to help justify such an assumption.
		...
		\begin{equation*}
			...
			,
		\end{equation*}
		where $X$ is the design matrix of the regression, and $v$ is a column vector of zeroes. Thus, if we assume we have a single observation
		such that $n=1$,we may take the inverse of the Fisher information matrix and isolate the entry related to the error variance $\sigma^2$. We see that
		...
		We let $I_{n} (\theta)$ denote the observed information matrix with regards to our specified linear regression model. With $X$ denoting the $n$ by $p$ design matrix, we see that
		...
		\begin{equation*}
			...
			\begin{bmatrix}
				...
				= -\frac{1}{n} I_n(\theta) .
		\end{equation*}
		Now let $x_i$ be the $i$th row of the design matrix $X$, and $y_i$ be the $i$th observation of the observation vector $y$. With this in mind, we can define the score components
		...
		\begin{equation*}
			...
			\end{split}
		\end{equation*}
		where $C_n(\hat{\theta})$ will be a $p+1$ by $p+1$ matrix that can be used as an estimator of the asymptotic variance-covariance matrix of $\hat{\theta}$. This estimator is robust to misspecification
		...
		For the remainder of this thesis, this estimator will be referred to as $\widehat{Var}[GOF]$ such that
		\begin{equation*}
		...
		\section{Bootstrap Test for Goodness-Of-Fit of a Linear Model} \label{sec:boot_test}
		...
		all assumptions of normal linear regression. If any of them are violated, the property $Var[GOF] \approx 2n$ is not guaranteed to hold. This allows the test to detect
		...
		and may use any bootstrap interval they so choose as long as it is theoretically justifiable. Additionally, the non-parametric bootstrap is employed in the algorithm detailed above to avoid further
		assumptions. However, it has been found that the residual bootstrap may also work just as well in this procedure, and one may decide to employ the test
		using this bootstrap method instead if so desired. However, all simulations presented in this thesis will use the non-parametric bootstrap flavor of the procedure.
		...
		whether these tests reject their null hypotheses of homoskedasticity will be recorded. This is to establish the ability of these tests to maintain a desired Type
		...
		null hypotheses of homoskedasticity are violated. However, he bootstrap test is able to reject its more general null hypothesis.
